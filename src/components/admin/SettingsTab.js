...
export default function SettingsTab({ setTab }) {
  const { session, setSession, showToast } = useAdminShell();
  ...
  const colorWell = /^#[0-9a-fA-F]{6}$/.test(draft.brandColor) ? draft.brandColor : "#2563EB";

  // Load current school settings when the tab opens
    ...
    try {
      const res = await fetch("/api/school", {
        ...
        body: JSON.stringify({
          ...
          sealUrl: draft.sealUrl,
          notificationRetentionDays: draft.notificationRetentionDays,
          reconcileDeletedReminders: draft.reconcileDeletedReminders,
    ...
    <>
      ...
      <div className="grid gap-8 p-6 lg:grid-cols-2">
        <div>
                ...
                <input
                  value={
                    draft.brandColor.startsWith("#")
                      ? draft.brandColor.slice(1)
                      : draft.brandColor
                  }
                  onChange={(e) => {
                    const v = e.target.value.replace(/[^0-9a-fA-F]/g, "").slice(0, 6);
                    setDraft((d) => ({ ...d, brandColor: v ? `#${v}` : "" }));
                  }}
                  onBlur={() => {
                    if (!/^#[0-9a-fA-F]{6}$/.test(draft.brandColor)) {
                      setDraft((d) => ({ ...d, brandColor: "#2563EB" }));
                    }
                  }}
                  placeholder="2563EB"
          ...
          <div className="mt-6">
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                min={1}
                max={3650}
                value={draft.notificationRetentionDays}
                onChange={(e) =>
                  setDraft((d) => ({
                    ...d,
          ...
          <div className="mt-6">
            <label className="flex cursor-pointer items-start gap-3">
              <input
                type="checkbox"
                checked={draft.reconcileDeletedReminders}
                onChange={(e) =>
                  setDraft((d) => ({
                    ...d,
          ...
          <div className="mt-6 border-t border-navy-100 pt-6">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <label className="block">
                ...
                <input
                  value={draft.bankDetails?.sortCode || ""}
                  onChange={(e) =>
                    setDraft((d) => ({
                      ...d,
            ...
            <label className="mt-3 block">
              ...
              <textarea
                value={draft.bankDetails?.otherInstructions || ""}
                onChange={(e) =>
                  setDraft((d) => ({
                    ...d,
        ...
        <div className="h-fit overflow-hidden rounded-xl border border-navy-200">
          ...
          <div className="p-5" style={{ backgroundColor: draft.brandColor }}>
