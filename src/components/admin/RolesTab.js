...
/**
 * Roles & Access tab — fully self-contained.
 * Manages its own staff list, role audit trail, and role-change logic.
 */
export default function RolesTab({ openReset }) {
  ...
  const { data: roleAudit = [] } = useTabFetch("/api/users/roles/audit", {
    enabled: true,
    transform: (d) => d.entries || [],
  });

  // Staff directory — one query per staff role
  useEffect(() => {
    Promise.all(
      MANAGED_ROLES.map((r) =>
  ...
  const ROLE_BADGES = {
    ...
    STUDENT: "bg-navy-50 text-navy-600 ring-navy-600/20",
  };
    ...
    <div className="mt-5 space-y-5 animate-fade-up">
      ...
      <div className="overflow-hidden rounded-2xl border border-navy-200/70 bg-white shadow-sm">
        ...
        <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
          {MANAGED_ROLES.map((role) => {
            ...
            return (
              <div key={role} className="rounded-xl border border-navy-100 bg-navy-50/40 p-4 transition hover:border-brand-200 hover:bg-brand-50/30">
                <div className="flex items-center justify-between gap-2">
                  <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-bold ring-1 ${ROLE_BADGES[role] || "bg-navy-100 text-navy-600"}`}>
                ...
                <div className="mt-3 space-y-3">
                  {summary.domains.map((d) => (
                    <div key={d.key}>
                      <p className="text-[10px] font-bold uppercase tracking-wider text-navy-400">{d.label}</p>
                      <ul className="mt-1 space-y-1">
                        {d.actions.map((a) => (
                          <li key={a.action} className="flex items-start gap-1.5 text-xs text-navy-700" title={a.action}>
        ...
        <div className="border-t border-navy-100 bg-navy-50/40 px-6 py-3 text-xs text-navy-500">
          Row-level scoping applies on top of this list — a teacher&apos;s actions cover only their assigned class arm, a parent only their own children.
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
                  ...
                  <tr key={u.id} className="border-b border-navy-50 transition hover:bg-navy-50/40">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="flex items-center gap-2 font-semibold text-navy-800">
                            {u.name}
                            {isYou && (
                              <span className="rounded-full bg-navy-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-navy-500">You</span>
                            )}
                          </p>
                    ...
                    <td className="px-6 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => openReset(u)} title={`Reset ${u.name}'s password`} className="rounded-lg p-1.5 text-navy-300 transition hover:bg-brand-50 hover:text-brand-600">
              ...
              {staffList.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-10 text-center text-navy-400">No staff accounts yet.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <div className="overflow-hidden rounded-2xl border border-navy-200/70 bg-white shadow-sm">
        <div className="border-b border-navy-100 px-6 py-4">
          ...
          <p className="text-sm text-navy-400">Every promotion or demotion — who did it, and when.</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead>
      ...
      {roleConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-navy-800">Confirm role change</h3>
            <p className="mt-2 text-sm text-navy-500">
              Change <strong>{roleConfirm.name}</strong> from{" "}
              <span className="font-semibold">{ROLE_LABELS[roleConfirm.from]}</span> to{" "}
              <span className="font-semibold">{ROLE_LABELS[roleConfirm.to]}</span>?
            </p>
            <div className="mt-5 flex justify-end gap-3">
              <button onClick={() => setRoleConfirm(null)} className="rounded-lg border border-navy-200 bg-white px-4 py-2 text-sm font-semibold text-navy-600 hover:bg-navy-50">
                Cancel
              </button>
              <button
                onClick={confirmRoleChange}
                disabled={roleSaving}
                className="inline-flex items-center gap-1.5 rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-500 disabled:opacity-60"
              >
